...
struct atomic_latency
{
    ...
    void sloop()
    {
        ...
        for (size_t i = 0; i < iters; i++) {
            uint y, x = sctr.load(std::memory_order_relaxed);
            do {
                y = rctr.load(std::memory_order_relaxed);
            } while (x != y);
	    std::atomic_thread_fence(std::memory_order_seq_cst);
            sctr.store(x+1, std::memory_order_relaxed);
        }
    ...
    void rloop()
    {
        ...
        for (size_t i = 0; i < iters; i++) {
            uint y, x = rctr.load(std::memory_order_relaxed);
            do {
                y = sctr.load(std::memory_order_relaxed);
            } while (x == y);
	    std::atomic_thread_fence(std::memory_order_seq_cst);
            rctr.store(y, std::memory_order_relaxed);
        }
